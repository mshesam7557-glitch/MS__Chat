...
def get_rtc_ice_servers():
    # Always keep public STUN first. TURN is appended only when all credentials are present.
    servers = [
    ...
    if urls and TURN_USERNAME and TURN_CREDENTIAL:
        servers.append({
            "urls": urls,
...
async def delete_message(
    ...
    with get_db() as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT sender,receiver,message,audio,media,status,message_type,reply_to,edited,deleted,group_id,created_at FROM messages WHERE id=%s",
                (message_id,),
            )
            row = cursor.fetchone()
    ...
    msg = {
        ...
        "group_id": row["group_id"], "message": row.get("message") or "",
        "audio": row.get("audio"), "media": row.get("media"),
        "message_type": row.get("message_type") or "text", "deleted": True
    }
        ...
        with get_db() as c:
            with c.cursor() as cur:
                if report_id is not None and str(report_id).strip():
                    ...
                    rr = cur.fetchone()
                    if not rr:
                        return {'success': False, 'messages': [], 'message': 'گزارش پیدا نشد.'}
